...
RUN cd /usr/local/ \
  && tomcat_major=$(echo ${tomcat_version} | cut -d '.' -f 1) \
  && wget http://apache.ip-connect.vn.ua/tomcat/tomcat-${tomcat_major}/v${tomcat_version}/bin/apache-tomcat-${tomcat_version}.tar.gz \
